...
1. In the pane on the left, in the **Instructions** text area, change the model's instructions to `You are an AI assistant that provides succinct answers to business expense-related questions.` **(1)**
...
1. At the top of the chat pane, use the **New chat** (&#128172;) **(2)** button to restart the conversation. This removes all conversation history.
...
1. In the pane on the left, expand the **Tools (2)** section if it's not already expanded.

1. In the **Add (3)** menu, select **Web search (4)** to add a *web search* tool.
...
1. Enter the prompt `Find me a credit card that's good for business expenses`.
    ...
    > **Note:** The web search tool is great for finding general information on the public Internet; but we need our agent to be able to get information from corporate expense policy documentation.

1. Open a new browser tab, enter the following URL in the address bar, press **Ctrl+S**, or right-click anywhere on the page, select **Save as**, and then save the file to your local computer.

    ```text
    https://aka.ms/expenses-txt
    ```
...
2. In the **Save As** dialog, select the **Downloads (1)** folder or another local folder, and then select **Save (2)**.
...
1. In the **Open** dialog, select the **Downloads (1)** folder, select the **expenses (2)** file, and then select **Open (3)**.

     ![](./media/lab2-07-10.png)
...
1. Open a new browser tab, enter the following URL in the address bar, and then wait for the Python environment and model to finish loading.
    ...
    ```text
    https://aka.ms/model-coder
    ```

    > **Note**: The first time the model loads, it may take a few minutes. If the model takes too long to load, select **Cancel** to start in **Basic mode**. You can switch between available models at any time from the **Mode** list.

    ![](./media/lab2-07-12.png)
...
1. When the model has loaded, change the **Blank Page (1)** sample to the **Simple chat (Responses API) (2)** sample; and review the code in the **Editor** pane. It should be similar to this:

    ![](./media/lab2-07-13.png)
